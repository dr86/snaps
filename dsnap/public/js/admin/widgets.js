    ...
    TheWidgets = {
        init: function () {
            ...
            avWidgets.children(".portlet").draggable({
				...
				{
		            widgets.find('.portlet-content').remove();
				}
			});
            /* sortable */
            widgets.sortable({
					...
					if( response.ok )
					{
						if( del ) /* aksi delete widget */
						{
							...
							*/
							...
							item.find( ".portlet-inside" ).append('<div id="response_msg" class="notification success">' + response.message + '</div>');
