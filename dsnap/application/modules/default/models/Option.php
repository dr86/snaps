...
class Model_Option extends My_Db_Object_Abstract
{
	...
	{
		parent::__construct($db, 'snap_options', 'option_id');
	...
	{
		return $this->_db->update('snap_options',
	 ...
	 * @return array 
	 */
	public static function GetWidgets($db, $options = array())
	{
		$select = self::_GetBaseQuery($db, $options);
		$data = $db->fetchAll($select);
		$options = parent::BuildMultiple($db, __CLASS__, $data);
		...
		foreach($options as $option)
		{
			$value = unserialize($option->option_value);
			// KEY : nama widget
			$widgets[$option->option_name] = array(
				'id' 			=> $option->getId(),
				'name' 			=> $option->option_name,
				'title' 		=> $value['title'],
				...
				'use_page' 		=> $value['use_page'],
				'use_menu' 		=> $value['use_menu'],
				'data' 			=> $value['data']
			);
		}
